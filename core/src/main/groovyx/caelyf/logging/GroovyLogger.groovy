 ...
 */
package groovyx.caelyf.logging
...
class GroovyLogger {
    ...
    static GroovyLogger forTemplateUri(String uri) {
        new GroovyLogger("caelyf.template.${uri[1..<uri.lastIndexOf('.')].replaceAll(/\//, '.')}", true)
    }

    static GroovyLogger forGroovletUri(String uri) {
        new GroovyLogger("caelyf.groovlet.${uri[1..<uri.lastIndexOf('.')].replaceAll(/\//, '.') - 'WEB-INF.groovy.'}", true)
    }
